# SEI-Tag
Extensão de uso individual, para facilitar o acompanhamento e gestão de processos na própria carga

Ferramenta elaborada a nível de hobby com IA, fique a vontade para contribuir com o aprimoramento.

A primeira versão injeta as tags na página de controle de processos para cada processo registrado, através do popup da extensão é possível importar ou exportar o arquivo CSV (base de dados), com as informações (Nº de processo, Tag1 e Tag2), edição das tags na própria extensão e adicionar novos processos.

A versão em produção 2.x, busca retirar a limitação da base de dados local e permitir que o mesmo banco de dados possa ser consultado através da extensão em diferentes dispositivos, e com adição do histórico do andamento do processo se a página ativa estiver exibindo um processo.

# A seguir as premissas da estrutura da nova versão 
...
4.3. Indicador textual. Ao lado do botão principal deverá constar indicador textual resumido do estado da integração, nos seguintes termos: Google conectado, em verde; pendente, em amarelo; e erro, em vermelho.
...
[Preparado usando GPT-5.4 Thinking]
